...
def eval_gt_scannetpp(gt_npy_folder: Union[str, Path] = None, feat_dir: Path=None, ouput_path: Path = None, label_name_list=None, suffix='') -> Dict:
    """
    ...
    """
    # gt_json_paths = sorted(glob.glob(os.path.join(str(gt_npy_folder), 'frame_*.json')))
    # img_paths = sorted(glob.glob(os.path.join(str(gt_npy_folder), 'frame_*.jpg')))
    # gt_npy_paths = sorted(glob.glob(os.path.join(str(gt_npy_folder), '*.npy')))
    to_eval_npy_paths = sorted(os.listdir(str(feat_dir[0]))) # choose one level to get name
    ...
    base_folder = os.path.basename(str(feat_dir[0]))
    # print("base_folder: ", base_folder)
    render_img_root = str(feat_dir[0]).replace(base_folder, 'gt')
    img_paths = [os.path.join(render_img_root, f'{img_name}.png') for img_name in img_names]
    ...
    for idx, to_eval_npy_path in enumerate(to_eval_npy_paths):
        ...
        save_path = ouput_path / 'gt' / to_eval_npy_path.split('.npy')[0] / f'{label_name}.jpg'
...
def lerf_localization(sem_map, image, clip_model, image_name, img_ann):
    ...
    valid_map = clip_model.get_max_across(sem_map)                 # 3xkx832x1264
    n_head, n_prompt, h, w = valid_map.shape
    # print("valid_map shape: ", valid_map.shape) # torch.Size([1, 12, 584, 876])
...
def evaluate(feat_dir, output_path, ae_ckpt_path, gt_npy_folder, mask_thresh, encoder_hidden_dims, decoder_hidden_dims, logger, label_name_txt_path):
    ...
    # gt_ann, image_shape, image_paths = eval_gt_lerfdata(Path(gt_npy_folder), Path(output_path))
    gt_ann, image_shape, image_paths = eval_gt_scannetpp(gt_npy_folder, feat_dir, Path(output_path), label_name_txt)
    ...
    for i in range(len(feat_dir)):
        ...
        # sorted(glob.glob(os.path.join(feat_dir[i], '*.npy')),
    ...
    for j, idx in enumerate(tqdm(eval_index_list)):
        ...
        with torch.no_grad():
            ...
            # print("sem_feat shape: ", sem_feat.shape) # 3 is levels 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gt_npy_folder", type=str, default=None)
    parser.add_argument("--label_name_txt", type=str, default=None)
    ...
    feat_dir = [os.path.join(args.feat_dir, dataset_name, f"feature_level_{i}", "test/ours_None/gt_npy") for i in range(1,4)] # gt_npy # renders_npy
    ...
    gt_npy_folder = args.gt_npy_folder
    ...
    evaluate(feat_dir, output_path, ae_ckpt_path, gt_npy_folder, mask_thresh, args.encoder_dims, args.decoder_dims, logger, args.label_name_txt)