...
def main():
    scene_folder = "/usr/bmicnas02/data-biwi-01/qimaqi_data/workspace/iccv_2025/GS_Transformer/data/scannet_full/data/09c1414f1b/dslr/"
    ...
    out_folder = os.path.join(scene_folder, "autoencoder_zero_shot_semseg")
    os.makedirs(out_folder, exist_ok=True)

    seg_files = sorted(glob.glob(os.path.join(scene_folder, 'language_features_dim3', "*_s.npy")))
    for seg_path in seg_files[:300]:
        # Derive the matching feature path
        base_name = os.path.basename(seg_path).replace("_s.npy", "")
        feat_path = os.path.join(scene_folder, 'language_features_dim3', base_name + "_f.npy")
